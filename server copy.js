...
// Initial stock price and GBM parameters
let currentPrice = 150; // Starting stock price
const drift = 0.0002; // Expected return (small positive drift)
const volatility = 0.01; // Volatility of the stock
const interval = 100; // Data emission frequency in milliseconds

// Helper function to simulate the next stock price using GBM
const generateNextStockPrice = (price) => {
...
io.on('connection', (socket) => {
  ...
  setInterval(() => {
    currentPrice = generateNextStockPrice(currentPrice);
    const newTimestamp = new Date().toISOString();
    socket.emit('stockData', { timestamp: newTimestamp, value: currentPrice });
  }, interval);
